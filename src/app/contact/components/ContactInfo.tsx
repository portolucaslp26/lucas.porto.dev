...
export default function ContactInfo() {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const items = entry.target.querySelectorAll('.ci-reveal');
  ...
  return (
    <div ref={ref} className="mt-10 space-y-6">
      {/* Contact details */}
      <div
        className="ci-reveal terminal-window"
      ...
      <div
        className="ci-reveal"
        style={{ opacity: 0, transform: 'translateY(20px)', transition: 'opacity 0.7s cubic-bezier(0.16,1,0.3,1) 100ms, transform 0.7s cubic-bezier(0.16,1,0.3,1) 100ms' }}
      >
